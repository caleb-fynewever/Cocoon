...
public class AimingCalculator {
	...
	public static double[] aimToPoint(
		...
		if (veloAim) {
			double[] shootOnMoveParams = getAdjustedShootOnMoveParams(yaw, distanceToTarget, chassisSpeeds);
	...
	private static double[] getAdjustedShootOnMoveParams(
			double uncompensatedYaw,
			double uncompensatedDistance,
			ChassisSpeeds chassisSpeeds) {
		Translation2d polarVelocity = new Translation2d(chassisSpeeds.vxMetersPerSecond,
				chassisSpeeds.vyMetersPerSecond).rotateBy(Rotation2d.fromDegrees(uncompensatedYaw));
		...
		double shotSpeed = uncompensatedDistance / ToFFactor - theta; // distance divided by time of flight minus the velocity toward the target
