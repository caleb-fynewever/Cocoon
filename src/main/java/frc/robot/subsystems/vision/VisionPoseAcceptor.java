...
public class VisionPoseAcceptor {
    ...
    public boolean shouldAccept(
        ...
        if (estimatedPose.getTranslation().getX() < -VisionConstants.FIELD_BORDER_MARGIN
                || estimatedPose.getTranslation().getX() > FieldConstants.FIELD_LENGTH
                        + VisionConstants.FIELD_BORDER_MARGIN
                || estimatedPose.getTranslation().getY() < -VisionConstants.FIELD_BORDER_MARGIN
                || estimatedPose.getTranslation().getY() > FieldConstants.FIELD_WIDTH
                        + VisionConstants.FIELD_BORDER_MARGIN) {
            return false;
        }
        ...
        if (isInAuto) {
            if (estimatedPose.getTranslation().toTranslation2d().getDistance(robotPose.getTranslation()) > VisionConstants.MAX_VISION_CORRECTION) {
                return false;
            }
        }
